...
void nanosleep_init() {
    port_byte_out(0x43, 0x14); // Config: channel 0, low byte, rate generator
    port_byte_out(0x40, 255); // Low byte initial count
}
...
void nanosleep(ui32 nanoseconds) {
    ...
    ui32 ticks_passed = 0;
    ui8 old_counter = port_byte_in(0x40);
    while (ticks_passed < ticks) {
        asm volatile("pause");
        ui8 counter = port_byte_in(0x40);
